...
def combine_data(file_path: str = None) -> bool:
    ...
    data.sort_values(by=['date','time','sym'], inplace=True, ignore_index=True)
...
def df2array3(df: pd.DataFrame, feature_len: int, df_columns: list)->np.array:
    """
    ...
    array_3d = df_pivot.values.reshape(df_pivot.shape[0], len(df_pivot.columns.levels[1]), -1)
    array_3d = np.swapaxes(array_3d, 0, 1)
    arr_features = array_3d[:, :, :feature_len] # 在这种写法中，必须要保证dataframe 的最后五列是labels
...
def export_data(df: pd.DataFrame, is_Train: bool = True,)->bool:
    if is_Train is True:
        ...
        feature_len = len(df_columns) - len(labels)  # 特征长度不包含labels，目的是把特征和标签切开
